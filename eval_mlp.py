...
def train_mlp(
    ...
    if change_val:
        X_num_real, X_cat_real, y_real, X_num_val, X_cat_val, y_val = tddpm_lib.read_changed_val(real_data_path, val_size=0.2)
    ...
    if eval_type == 'merged':
        ...
        if not change_val:
            X_num_real, X_cat_real, y_real = tddpm_lib.read_pure_data(real_data_path)
        X_num_fake, X_cat_fake, y_fake = tddpm_lib.read_pure_data(synthetic_data_path)
        ...
        X_num, X_cat, y = tddpm_lib.read_pure_data(synthetic_data_path)

    elif eval_type == 'real':
        print('loading real data...')
        if not change_val:
            X_num, X_cat, y = tddpm_lib.read_pure_data(real_data_path)
    else:
        raise "Choose eval method"

    if not change_val:
        X_num_val, X_cat_val, y_val = tddpm_lib.read_pure_data(real_data_path, 'val')
    X_num_test, X_cat_test, y_test = tddpm_lib.read_pure_data(real_data_path, 'test')
    ...
    X = tddpm_lib.concat_features(D)

    X["train"], D.y["train"] = shuffle(X["train"], D.y["train"], random_state=seed)
    print(f'Train size: {X["train"].shape}, Val size {X["val"].shape}')

    if params is None:
        params = tddpm_lib.load_json(f"/home/rototo/tab-diffusion/tuned_models/mlp/{Path(real_data_path).name}_cv.json")
    ...
    if parent_dir is not None:
        tddpm_lib.dump_json(report, os.path.join(parent_dir, "results_mlp.json"))
