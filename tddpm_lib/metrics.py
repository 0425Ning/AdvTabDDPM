...
class MetricsReport:
    ...
    def get_metric(self, split: str, metric: str) -> float:
        # # add for metric
        # # print(f"Available splits: {self._res.keys()}")
        # if split in self._res and metric in self._res[split]:
        #     return self._res[split][metric]
        # else:
        #     # print(f"Metric '{metric}' not found for split '{split}'")
        #     return None
        
        return self._res[split][metric] # modify for metric
...
class SeedsMetricsReport:
    ...
    def get_mean_std(self) -> dict:
        ...
        # print('report: ', self._reports[0]) # add for metric
        # for split in self._reports[0].keys(): # modify for metric # for split in self._reports[0].get_metrics_names():
        #     for metric in self._reports[0].keys(): # modify for metric # for metric in self._reports[0].get_metrics_names():
        #         res[split][metric] = [x['metrics_report'].get_metric(split, metric) for x in self._reports] # modify for metric # res[split][metric] = [x.get_metric(split, metric) for x in self._reports]
...
def calculate_metrics(
    ...
    if task_type == TaskType.REGRESSION:
        ...
        result = cast(
            Dict[str, Any], skm.classification_report(y_true, labels, output_dict=True, zero_division=0) # modify # Dict[str, Any], skm.classification_report(y_true, labels, output_dict=True)
        )
        if task_type == TaskType.BINCLASS:
