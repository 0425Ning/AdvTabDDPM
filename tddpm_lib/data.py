...
"""
Returns the number of unique categories for each feature in a given dataset.

Parameters:
X (Union[torch.Tensor, np.ndarray]): A 2D tensor or array where rows are samples and columns are features.

Returns:
List[int]: A list of integers representing the number of unique categories for each feature.
"""
def get_category_sizes(X: Union[torch.Tensor, np.ndarray]) -> List[int]:
    # Convert the input to a list of lists, where each inner list is a feature column
    XT = X.T.cpu().tolist() if isinstance(X, torch.Tensor) else X.T.tolist()
    # Return the number of unique categories for each feature column
...
class Dataset:
    ...
    """
    Returns the number of unique categories for each categorical feature in the specified dataset part.
    
    Parameters:
    part (str): The dataset part to get the category sizes for (e.g., 'train', 'val', 'test').
...
def normalize(
    ...
    if normalization == 'standard':
        ...
        normalizer = sklearn.preprocessing.QuantileTransformer(
            ...
            subsample=1000000000, # modify # 1e9
            random_state=seed,
        )
        # noise = 1e-3
        # if noise > 0:
...
def cat_encode(
    ...
    if encoding is None:
        ...
        for k, v in X.items():
            try:
                transformed = encoder.transform(v)
                print(f"Successfully transformed {k}")
            except Exception as e:
                print(f"Error in transforming {k}: {e}")
...
def transform_dataset(
    ...
    if dataset.X_num is not None:
        dataset = num_process_nans(dataset, transformations.num_nan_policy) # process the missing data of X_num
    ...
    if X_num is not None and transformations.normalization is not None:
        X_num, num_transform = normalize( # X_num normalizaion
            X_num,
    ...
    if dataset.X_cat is None:
        ...
        X_cat = cat_process_nans(dataset.X_cat, transformations.cat_nan_policy) # process the missing data of X_cat
        if transformations.cat_min_frequency is not None:
            X_cat = cat_drop_rare(X_cat, transformations.cat_min_frequency)
        X_cat, is_num, cat_transform = cat_encode( # X_cat categorical encoding
            X_cat,
    ...
    # add
    # transformations.y_policy:  default
    # dataset.task_type:  multiclass
